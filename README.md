...
The command performs one review only and appends the validated result to `/secure/path/review-request-reviews.jsonl` by default. Use `--audit-log /secure/path/reviews.jsonl` to select another existing directory. Audit records are written as durable, append-oriented JSONL at mode `0600`; unsafe symlinks/hardlinks, out-of-policy provider metadata, and persistence errors fail closed. This local file is not tamper-evident against another process with access to the same OS account; stronger integrity requires an external trust anchor or OS-enforced append-only storage. It does not run commands, expose tools to the reviewer, mutate GitHub, merge, deploy, or automatically apply fixes. On any failure it prints only `live review failed closed`; it never prints the API key.
